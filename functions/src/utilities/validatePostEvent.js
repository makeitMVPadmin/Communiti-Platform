...
function validatePostEvent(body) {
  ...
  if (!isValidString(title))
    throw {
      code: 400,
      error: new Error(
        "Event Title cannot be an empty string please provide a valid title. E.g 'Cloud Summit'!"
      ),
    };
  ...
  if (!isValidString(description))
    throw {
      code: 400,
      error: new Error(
        "Event Description is a required field, please provide a valid description for this event."
      ),
    };
  ...
  if (!isValidStartTime(date, startTime, endTime))
    throw {
      code: 400,
      error: new Error(
  ...
  if (isValidString(timezone) && !(timezone in TIMEZONES))
    throw {
      code: 400,
      error: new Error(
